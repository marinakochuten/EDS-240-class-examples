...
mko_clean |>
  mutate(month_name = factor(month_name, levels = month.name)) |>   # Reorder month names so they aren't alphabetical
...
ggplot(dummy_data, aes(x = value, fill = group)) +
  ...
  # look at all of the data points and their distribution at the bottom of the plot!
...
ggplot(mko_clean, aes(x = Temp_bot, y = month_name, fill = after_stat(x))) +
  # rel_min_height to limit long tails on ridges, scale = to update how much the curves overlap
  geom_density_ridges_gradient(rel_min_height = 0.01,
                               scale = 3) +
  scale_fill_gradientn(colors = c("#2C5374","#849BB4", "#D9E7EC", "#EF8080", "#8B3A3A")) +
  # reorder y axis variable to be in month order - rev() reverses so Jan is on top
...
ggplot(penguins, aes(x = species, y = body_mass_g)) +
  ...
  geom_jitter(alpha = 0.5, size = 0.2, width = 0.2)
```
